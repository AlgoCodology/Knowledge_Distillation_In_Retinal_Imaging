...
class CustomDataGenerator(tf.keras.utils.Sequence):
    def __init__(self, dataframe, directory, x_col, y_col, target_size, batch_size, class_mode, subset=None, mixup=False):
...
teacher_selected = 'b7' #'ensemble_v3' , 'b4' , 'b5' , 'b6'
...
if student_selected == 'mobnet':
    ...
    base_model = EfficientNetB0(input_shape=(512, 512, 3), include_top=False, weights=None, pooling='avg')
...
#         log dictionary dict_keys(['loss', 'categorical_accuracy', 'hard_loss', 'soft_loss', 'val_loss', 'val_categorical_accuracy', 'val_hard_loss', 'val_soft_loss', 'lr'])
    def on_epoch_end(self, epoch, logs=None):
        ...
        print('LEARNING_RATE:', self.model.optimizer.lr)
...
class LearningRateScheduler(Callback):
    def __init__(self, warmup_steps, total_steps, peak_lr, min_lr):
        super().__init__()
...
#         print("CurrentStep:",current_step)
        if current_step < self.warmup_steps:
            lr = (current_step / self.warmup_steps) * self.peak_lr
        else:
            progress = (current_step - self.warmup_steps) / (self.total_steps - self.warmup_steps)
            lr = self.min_lr + 0.5 * (self.peak_lr - self.min_lr) * (1 + math.cos(progress * math.pi))
        tf.keras.backend.set_value(self.model.optimizer.lr, lr)
#         print("Learning rate:", lr)

# Create an instance of the LearningRateScheduler callback
warmup_steps = 40*my_steps_per_epoch
total_steps = num_epochs*my_steps_per_epoch
peak_lr = 0.01
min_lr = 0.000001
lr_scheduler = LearningRateScheduler(warmup_steps, total_steps, peak_lr, min_lr)
...
callbacks_= [early_stop, tboard, csv_logger, bestepochsaver, lr_scheduler] #reduce_lr,
...
new_experiment = {'exp_no': folders_count, 'student': student.name,
                  ...
                  'max_epochs': num_epochs, 'traingenerator_size': train_generator.__len__()*train_generator.batch_size,
                  'trainsteps': my_steps_per_epoch, 'trainvaltest_batchsize':train_generator.batch_size,
                  'validationgenerator_size': validation_generator.__len__()*validation_generator.batch_size,
