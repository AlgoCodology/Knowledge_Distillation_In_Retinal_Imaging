...
student_model = tf.keras.models.Model(inputs=student_model.input, outputs=student_predictions, name = 'EffNet_B0_imagenet_Glaucoma')
student_name = 'EffNet_B0_imagenet_Glaucoma'
...
checkpoint = ModelCheckpoint(student_name+'_exp2.h5', monitor='val_loss', save_best_only=True, verbose=1, mode='min', baseline=None, restore_best_weights=True)
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=10, min_lr=0.000002, verbose=1)
# Train the model
tboard = keras.callbacks.TensorBoard(log_dir='./logs/exp_student/'+student_name+'_exp2', write_graph=False, profile_batch=0)
student_model.fit(train_generator, steps_per_epoch=400,#steps_per_epoch=train_generator.n // train_generator.batch_size,
                            epochs=200, validation_data=validation_generator,
                            validation_steps = 400, #validation_steps=validation_generator.n // validation_generator.batch_size,
